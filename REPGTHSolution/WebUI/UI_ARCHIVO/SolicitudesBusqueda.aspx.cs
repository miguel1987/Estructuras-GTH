...
{
    public partial class SolicitudesBusqueda : PageBaseClass
    {
        Guid USUARIO;
        ...
        protected void rgSolicitudes_ItemDataBound(object sender, GridItemEventArgs e)
        {
            ...
            try
            {
                ...
                if (item != null)
                {
                    String CODIGO = DataBinder.Eval(e.Item.DataItem, "CODIGO").ToString();
                    String ESTADO = DataBinder.Eval(e.Item.DataItem, "ESTADO_DESCRIPCION").ToString();
                    ImageButton btnPrint = (ImageButton)item["Print"].Controls[0];

                    if (btnPrint != null)
                    {
                        if (ESTADO != "Pendiente Aprobación")
                        {
                            string url = "../UI_REPORTES/Solicitud.aspx?heSolicitudCodigo=" + CODIGO;

                            btnPrint.Attributes.Add("OnClick", "window.open('" + url + "');");
                        }
                        else
        ...
        protected void rgSolicitudes_DeleteCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                BL_SOLICITUD.ActualizarEstadoSolicitud(Guid.Parse(values["ID"].ToString()), (int)BE_SOLICITUD.SOLICITUD_ESTADO.Eliminada, USUARIO, string.Empty);
            }
            catch (Exception ex)
            {
                lblMensaje.Text = "Error al Eliminar Solicitud:" + ex.ToString();
